---
title: Contacto
description: Cómo contactar al Club Suzuki GTi
---

# Contacto

## Para miembros del club

La vía principal es el **[grupo Telegram](comunidad/telegram.md)**. Una vez dentro, puedes:
- Hacer preguntas técnicas
- Comprar/vender repuestos
- Coordinar eventos
- Compartir tu build

## Para contribuir contenido

Si tienes información valiosa para el archivo (manuales antiguos, fotos históricas, tutoriales de reparación, datos de repuestos):

### Opción A — Pull Request en GitHub
Si sabes Git: forkea el repo, crea tu rama, envía PR.
- Repo: [github.com/Clubsuzukigti/suzukigti.github.io](https://github.com/Clubsuzukigti/suzukigti.github.io)
- Guía: ver `CONTRIBUTING.md` del repo

### Opción B — Telegram
Postea en el grupo principal y un admin convertirá tu aporte en página del sitio.

### Opción C — Email directo
**ferpezlro@gmail.com** — admin principal del proyecto

## Para prensa, medios, otros clubs

Si representas un medio especializado, otro club de Suzuki, o quieres colaborar institucionalmente:

- 📧 **Email:** ferpezlro@gmail.com
- 📘 **Facebook:** [Suzuki Lifestyle](https://www.facebook.com/suzukilifestyle)

## Para temas legales / derechos de autor

Los manuales originales preservados aquí son **copyright Suzuki Motor Corporation** (1990-2003). Los mantenemos para:
1. Preservación histórica de vehículos discontinuados
2. Uso personal de propietarios actuales en restauración/mantenimiento
3. Material educativo no comercial

Si representas a Suzuki Motor Corp o tienes preocupaciones de derechos:
- **Email:** ferpezlro@gmail.com
- Responderemos en 48 horas hábiles
- Retiraremos contenido específico si se solicita formalmente

## Para hacer mantenimiento del sitio

Admins actuales:
- **Fernando** (admin principal)
- Más admins se sumarán conforme crezca el club

Si quieres ayudar a mantener el sitio (no requiere saber código necesariamente — también puedes ayudar con edición de contenido), contacta vía Telegram o email.

---

## Privacidad

- **No recopilamos** datos personales de visitantes del sitio
- **No usamos** rastreadores publicitarios
- **Cookies:** solo las técnicas necesarias para preferencia de idioma + tema oscuro/claro
- Analítica privacy-friendly (sin datos identificables) puede activarse en el futuro

Política completa en [PRIVACY.md del repo](https://github.com/Clubsuzukigti/suzukigti.github.io/blob/main/PRIVACY.md).
